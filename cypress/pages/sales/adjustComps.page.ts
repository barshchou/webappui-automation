...
class AdjustCompsPage extends BasePage {
    ...
    getAdjustmentEditNameButton(prevName: string) {return cy.get(`[title='${prevName}'] button[aria-label=edit] svg`);}
    getAdjustmentDeleteButton(name: string) {return cy.get(`[title='${name}'] button[aria-label=delete] svg`);}
