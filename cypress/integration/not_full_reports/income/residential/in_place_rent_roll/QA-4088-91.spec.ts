...
describe(`[QA-4088], [QA-4089], [QA-4090] [QA-4091] 'What time period should rent PSF be based on?' 
radio buttons and label functionality`,
{ tags: [ "@income", "@residential", "@in_place_rent_roll" ] }, () => {
    ...
    it("[QA-4088]", () => {
        cy.stepInfo(`1. Verify if "What time period should rent PSF be based on?" question with 2 radio buttons 
        ...
        Income._Residential.InPlaceRentRoll.verifyPerUnitSFRadioCheck(false)
            .checkPerUnitSquareFootage()
            .Page.getPSFRadio(testData.psfRadioValuePerAnnually).should("exist");
    ...
    it("[QA-4089]", () => {
        cy.stepInfo(`1. Verify if "What time period should PSF rent be based on?" question is not displayed 
                if "No" radio button is selected in "Do you know per unit square footage?"`);
        Income._Residential.InPlaceRentRoll.checkPerUnitSquareFootage(false)
            .Page.radioButtonLabelByText(testData.labelText).should("not.exist");
    });

    it("[QA-4090]", () => {
        cy.stepInfo(`1. "Verify if  'Per Year' radio button is selected by default" `);
        Income._Residential.InPlaceRentRoll.checkPerUnitSquareFootage()
            .Page.getPSFRadio(testData.psfRadioValuePerAnnually).should("be.checked");
    });

    it("[QA-4091]", () => {
        cy.stepInfo(`1. Verify if User selects 'Per Year' time period PSF rent based on -> 
        'Rent SF' is column name in In-Place RR table`);
        Income._Residential.InPlaceRentRoll.checkPerUnitSquareFootage(false)
            .checkPerUnitSquareFootage()
