...
describe(" [QA-5761] [QA-5762] Appraiser's Forecast value for General & Administrative is correctly converted to Per SF/Unit value",
    { tags: [ "@income", "@expense_forecast" ] }, () => {
        ...
        it("[QA-5761] Per Unit Appraiser's Forecast value for General & Administrative is correctly converted to Per SF value", () => {
            ...
            cy.stepInfo("1. Go to Expense Forecast and make sure that Per Unit radiobutton is selected for General & Administrative card");
            _NavigationSection.Actions.navigateToExpenseForecast();
            Income._ExpenseForecastActions.chooseForecastItemBasis(testData.expenseForecastGeneralAdministrativeFixture('unit'))
            ...
            Income._ExpenseForecastActions.enterForecastItemForecast(testData.expenseForecastGeneralAdministrativeFixture('unit'));
        ...
        it("[QA-5762] Per SF Appraiser's Forecast value for General & Administrative is correctly converted to Per Unit value", () => {

            cy.stepInfo("1. Go to Expense Forecast and make sure that Per SF radiobutton is selected for General & Administrative card");
            _NavigationSection.Actions.navigateToExpenseForecast();
            Income._ExpenseForecastActions.chooseForecastItemBasis(testData.expenseForecastGeneralAdministrativeFixture('sf'))
                .verifyForecastItemBasis(testData.expenseForecastGeneralAdministrativeFixture('sf'));

            cy.stepInfo("2. Fill in Appraiser's Forecast field for General & Administrative card");
            Income._ExpenseForecastActions.enterForecastItemForecast(testData.expenseForecastGeneralAdministrativeFixture('sf'));
