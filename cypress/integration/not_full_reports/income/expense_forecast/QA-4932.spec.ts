...
describe("Comparable Min, Max, Avg values for Repairs & Maintenance Per Unit are correctly calculated and displayed", () => {
  ...
  it("Test body", { tags: "@snapshot_test" }, () => {
    ...
    cy.stepInfo(
      "2. Add several comps (via Search, Filter or Add blank column)"
    );
    testData.comparables.forEach((comp, index) => {
      Income.ComparableExpenses.Actions.clickAddBlankColumnButton()
        ...
        .enterCellDollarValueByColumnIndex(
          Income.ComparableExpenses.Page.repairsCells,
    ...
    cy.stepInfo(
      "3. Make sure that Repairs & Maintenance and Residential Units fields are filled in for all added columns and save changes"
    );
    ...
    cy.stepInfo(`
      ...
      `);
    Income.ExpenseForecast.Actions.verifyForecastItemCompMin(
      testData.repairsItem,
      testData.comparables
    )
