...
describe("[QA-5771] Historical expense Payroll & Benefits Per SF is correctly calculated and displayed",
    ...
    it("[QA-5771]", () => {
        ...
        cy.stepInfo(`4.1 Check historical expenses values for Payroll & Benefits card. They should be calculated 
                     for each expense type as: [Expense Period type]Payroll & Benefits / GBA`);
        Income._ExpenseForecastActions.Actions.verifyForecastItemByExpensePeriodType(testData.actualPayrollAndBenefitsItem, testData.buildingDescription, "Actual")
            .verifyForecastItemByExpensePeriodType(testData.t12PayrollAndBenefitsItem, testData.buildingDescription, "Actual T12")
            .verifyForecastItemByExpensePeriodType(testData.historicalPayrollAndBenefitsItem, testData.buildingDescription, "Annualized Historical")
            .verifyForecastItemByExpensePeriodType(testData.ownerProjectionPayrollAndBenefitsItem, testData.buildingDescription, "Owner's Projection")
            .hideHeader()
            .clickSaveButton()
            .verifyProgressBarNotExist();

        cy.stepInfo("4.2 Check historical expenses values for Payroll & Benefits card. They should be correctly displayed on slidebars");
