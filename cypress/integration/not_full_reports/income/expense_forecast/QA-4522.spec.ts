    ...
    it("Test body", () => {
        ...
        testData.comparables.forEach((comp) => {
            Income._ComparableExpenses.clickAddBlankColumnButton()
                ...
                .enterCellDollarValueByColumnIndex(Income._ComparableExpenses.Page.getUnifiedEditableAndTotalCells(testData.insuranceItem.name),
                    comp.insurance)
                .enterResidentialUnitsByColumnIndex(comp.resUnits);
        });

        cy.stepInfo("3. Go to Expense Forecast and make sure that Per Unit radiobutton is selected for Insurance card");
        ...
        Min, Max and Avg of range of values [Comp_Insurance / Square Feet]`);
        ...
        Income._ExpenseForecastActions.matchElementSnapshot(
            Income._ExpenseForecastActions.Page.forecastItemCard(
                Income._ExpenseForecastActions.getItemNameForAverage(
                    testData.insuranceItem.name)).scrollIntoView(), 
                    testData.insuranceCardSnapshotName, { padding: [ 10, 100 ], updatePassedSnapshot: true }
        );
