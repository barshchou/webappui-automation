...
describe("[QA-5769] [QA-5770] Appraiser's Forecast value for Replacement Reserves is correctly converted to Per SF/Unit value",
    { tags: [ "@income", "@expense_forecast" ] }, () => {
        ...
        it("[QA-5769] Per Unit Appraiser's Forecast value for Replacement Reserves is correctly converted to Per SF value", () => {
            ...
            cy.stepInfo("1. Go to Expense Forecast and make sure that Per Unit radiobutton is selected for Replacement Reserves card");
            _NavigationSection.Actions.navigateToExpenseForecast();
            Income._ExpenseForecastActions.chooseForecastItemBasis(testData.expenseForecastReplacementsReservesFixture('unit'))
            ...
            Income._ExpenseForecastActions.enterForecastItemForecast(testData.expenseForecastReplacementsReservesFixture('unit'));
        ...
        it("[QA-5770] Per SF Appraiser's Forecast value for Replacement Reserves is correctly converted to Per Unit value", () => {

            cy.stepInfo("1. Go to Expense Forecast and make sure that Per SF radiobutton is selected for Replacement Reserves card");
            _NavigationSection.Actions.navigateToExpenseForecast();
            Income._ExpenseForecastActions.chooseForecastItemBasis(testData.expenseForecastReplacementsReservesFixture('sf'))
                .verifyForecastItemBasis(testData.expenseForecastReplacementsReservesFixture('sf'));

            cy.stepInfo("2. Fill in Appraiser's Forecast field for Replacement Reserves card");
            Income._ExpenseForecastActions.enterForecastItemForecast(testData.expenseForecastReplacementsReservesFixture('sf'));
