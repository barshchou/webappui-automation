...
describe("[QA-4586] Verify the Commercial Stabilized Rent Roll table", 
    ...
    it("Test body", () => {
        ...
        Income._CommercialManager.InPlaceRentRoll.chooseListLeaseStatuses(testData.leaseStatusesOccupied, testData.numberOfCommercialUnits);
        ...
        testData.leaseStatusesOccupied.forEach((leaseStatus, index) => {
            Income._CommercialManager.StabilizedRentRoll.verifyLeaseStatusByRow(leaseStatus)
                ...
                .verifyAnnualRentCellPerSFBasisByRow(testData.rentsPsf[index], testData.listOfUnitsSF[index], testData.unitsOfMeasure, index)
                .verifyMonthlyRentPerSFByRow(testData.rentsPsf[index], testData.listOfUnitsSF[index], testData.unitsOfMeasure, index)
        ...
        _NavigationSection.navigateToCommercialInPlaceRentRoll();
        Income._CommercialManager.InPlaceRentRoll.chooseListLeaseStatuses(testData.leaseStatusesVacant, testData.numberOfCommercialUnits);
        ...
        testData.leaseStatusesMixed.forEach((leaseStatus, index) => {
                if (leaseStatus != "Vacant") {
                    Income._CommercialManager.InPlaceRentRoll.enterRentPerSFAnnuallyByRowNumber(testData.rentsPsf[index], index);
                }
            });
        ...
        testData.leaseStatusesMixed.forEach((_unit, index) => {
            if (testData.leaseStatusesMixed[index] == "Vacant") {
                ...
                Income._CommercialManager.StabilizedRentRoll.verifyRentPsfAnnuallyByRow(testData.rentsPsf[index], index);
            }
        });
