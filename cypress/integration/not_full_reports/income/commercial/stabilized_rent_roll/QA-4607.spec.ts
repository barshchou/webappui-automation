...
describe("Verify the Commercial Stabilized Rent Roll table", 
    ...
    it("[QA-4607]", () => {
        ...
        testData.leaseStatuses.forEach((_unit, index) => {
            Income._CommercialManager.StabilizedRentRoll
                .verifyAnnualRentCellPerSFBasisByRow(testData.marketRentConclusion, testData.listOfUnitsSF[index], testData.unitsOfMeasure, index);
        });

        cy.stepInfo(`11. Verify the Monthly Rent is calculated correctly per formula: (Rent PSF col.* SF col.)/12.`);
        testData.leaseStatuses.forEach((_unit, index) => {
            Income._CommercialManager.StabilizedRentRoll
                .verifyMonthlyRentPerSFByRow(testData.marketRentConclusion, testData.listOfUnitsSF[index], testData.unitsOfMeasure, index);
        });
