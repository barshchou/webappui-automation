...
describe("Subject column calculations", { tags: [ "@income", "@commercial", "@rent_comps" ] }, () => {
    ...
    it("All units are occupied and have rent PSF filled in", () => {
        ...
        Income._CommercialManager.InPlaceRentRoll.chooseListLeaseStatuses(testData.allOccupiedStatuses, testData.unitsNumber)
        ...
        Income._CommercialManager.RentComps.verifyComputedSubjectColumn(testData.rentPSFs, testData.allOccupiedStatuses);
    });
    ...
    it("Some Occupied units don't have rent PSF filled in", () => {
        ...
        cy.stepInfo("Navigate to commercial rent comps, verify computed subject column");
        _NavigationSection.navigateToCommercialRentComps();
        Income._CommercialManager.RentComps.verifyComputedSubjectColumn(testData.someZeroRentPSFs, testData.allOccupiedStatuses);
    });

    it("At least one unit is Vacant, others are Occupied and have rent PSF filled in", () => {
        cy.stepInfo("Fill lease statuses, enter rentPSF values");
        Income._CommercialManager.InPlaceRentRoll.chooseListLeaseStatuses(testData.oneVacantStatus, testData.unitsNumber)
            .enterListPerSF(testData.oneVacantStatus, testData.someZeroRentPSFs);
        cy.stepInfo("Navigate to commercial rent comps, verify computed subject column");
        _NavigationSection.navigateToCommercialRentComps();
        Income._CommercialManager.RentComps.verifyComputedSubjectColumn(testData.someZeroRentPSFs, testData.oneVacantStatus);
    });

    it("All units are Vacant", () => {
        cy.stepInfo("Fill lease statuses");
        Income._CommercialManager.InPlaceRentRoll.chooseListLeaseStatuses(testData.allVacantStatuses, testData.unitsNumber);
        cy.stepInfo("Navigate to commercial rent comps, verify computed subject column");
        _NavigationSection.navigateToCommercialRentComps();
    ...
    it("Vacant units were previously saved as Occupied and had rent PSF filled in", () => {
        ...
        _NavigationSection.navigateToCommercialRentComps();
        Income._CommercialManager.RentComps.verifyComputedSubjectColumn(testData.rentPSFs, testData.oneVacantStatus);
    });
