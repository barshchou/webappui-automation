...
describe(`[QA-5157] [Sales > Find Comps] "Date Sold" sorting is selected by default for sales comps`,
    { tags: ["@sales", "@find_comps", "@comp_plex"] }, () => {
        ...
        (comps added via map search )`);
            ...
            // .selectfilterSalePeriodValue('Last 3 Months')
            //    //   cy.wait(10000)
            ...
            cy.stepInfo(`2.Verify that when "Date Sold" option in Sort dropdown is selected comps are sorted in the next order:
- In-Contract at the top
- Listing
- date sold from most to least recent 
(comps created by user)`);
           ...
           cy.stepInfo(`3.Verify that when "Date Sold" option in Sort dropdown is selected comps are sorted in the next order:
           - In-Contract at the top
           - Listing
           - date sold from most to least recent 
