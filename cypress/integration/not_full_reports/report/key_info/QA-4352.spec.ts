...
describe("[QA-4352] Prefill Inspection Date from Salesforce (there is no Inspection Date in the Salesforce)",
    ...
    it("Test body", () => {
        cy.stepInfo("2. Go to Report → Key Info → Engagement tab and verify the values in the Inspection Date and the Date of Valuation are empty");
        _NavigationSection.navigateToReportInformation();
        testData.namesInputByQA.forEach(name => {
            Report._KeyInfo.Page.getDateInputByQA(name).should("be.empty");
        });
        
        cy.stepInfo(`3. Go to Report → Key Info → Engagement tab and verify that check-box "My Date of Valuation (As Is) 
            date is different from my Inspection Date" is unchecked`);
