...
describe(`[QA-4718-19] Verify the "Linked" chips dropdown in the new narrative component for Property Rights Appraised and Definition of Market Value sections`,
    ...
    it("Test body", () => {
        let i = 0;
        let j = 0;
        for (; i < testData.typesReport.length; i++) {
            cy.stepInfo("Login, create report");
            createReport(reportCreationFixture(testData.typesReport[i], `_${i + 1}`));
            ...
            Report._KeyInfo.Page.formEditBtn(0).click();
            for(; j < testData.chips.length; j++) {
                Report._KeyInfo.enterPropertyRightsAppraisedComment(`=${testData.chips[j].typeSuggestValue}`, false, false, false);
                Report._KeyInfo.clickNarrativeSuggestions(testData.chips[j].suggestionName);
                Report._KeyInfo.Page.textBoxPropertyRightsAppraised.should("include.text", testData.chips[j].verifySuggest);
                Report._KeyInfo.enterDefinitionMarketValue(`=${testData.chips[j].typeSuggestValue}`, false, false, false);
                Report._KeyInfo.clickNarrativeSuggestions(testData.chips[j].suggestionName, 1);
                Report._KeyInfo.Page.textBoxPropertyRightsAppraised.should("include.text", testData.chips[j].verifySuggest);
            }
            ...
            ReviewExport.generateDocxReport().waitForReportGenerated()
                .downloadAndConvertDocxReport(reportCreationFixture(testData.typesReport[i], `_${i + 1}`).reportNumber);
            deleteReport(reportCreationFixture(testData.typesReport[i], `_${i + 1}`).reportNumber);
            j = 0;
        }
