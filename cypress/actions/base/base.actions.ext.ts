...
export default class BaseActionsExt<T extends BasePage> extends BaseActions {
    ...
    enterFormCommentTextBox(name: string, text: string, isClickHeader = true, 
        ...
        this.Page.formCommentTextBox(name).realClick({ position: option.position }).realType(text);
        if (isClickHeader) {
    ...
    waitForUrl(route: Utils.Routes, _timeout?: number) {
        cy.url({ timeout: _timeout }).should("include", route);
        return this;
    }
